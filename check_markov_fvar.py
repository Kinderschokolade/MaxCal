	...
	for i in range(ms):
		for j in range(ms):
			if (T[i,j] > eps and T[j,i] > eps):
				W[i,j] =  T[i,j] * math.exp(gamma/2.*(r[i,j]+r[j,i]) + (Sprod[i,j]-np.log(T[i,j]/T[j,i]))/2. )
			else:
				W[i,j] =  1 * math.exp(gamma/2.*(r[i,j]+r[j,i]) + (Sprod[i,j]-1.)/2. )
...
for k in range(ms):
	for l in range(ms):
		...
		if (np.abs(k-l) <( ms /2)): # why do I change sign here (compared to reweighting)
			if (l > k):
				Sp[k,l] = ( np.sqrt(math.pi/2.)*(erf())  + potential[k,1] - potential[l,1] ) / Temperature
			else:
...
#					mfpt_ar[t][k] = mfpt(L,target = target, origin = origin)
		...
		np.savetxt("/data/isilon/bause/single_particle/"+str(int(tau))+"/T/ratio_check_"+str(ident)+".dat",ratio_check)
...
#		np.savetxt('/data/isilon/bause/single_particle/MFPT/markovhist_'+str(int(tau))+"_"+str(ident)+'.dat',(np.transpose(MFPT_hist)))
		...
		for k in range(ms):
			for t in range(ms):
				Omega[k,t] = np.sqrt(L[k,t] * L[k,t] ) #* np.exp((potential[k,1] + potential[t,1] )/(-2*Temperature))
