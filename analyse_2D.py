    ...
    for i in range(0,shape[0]*shape[1]):   #get transition matrix
        for j in range(0,shape[0]*shape[1]):            
            if p[i+shape[0]*shape[1]*shape[0]*shape[1]+1] != 0:
                transition_matrix[i][j] = count_matrix[j][i]/p[i+shape[0]*shape[1]*shape[0]*shape[1]+1] #count matrix devided by the row sum
            else:
...
def get_eigen(data_index, lagtime):
    ...
    eigen_lst = sorted(eigen_lst, key=lambda tup: tup[0], reverse=True) #sort everything by the size of the eigenvalues
...
def plot_eigenvectors(data_index, lagtime, ew=1, left=True, right = False, imag = False): #choose which eigenvectors (ew=?), right\left shall be plotted and if the imaginary parts shall be plotted aswell
    shape = get_shape(data_index)
    w,vl,vr = get_eigen(data_index, lagtime)
...
def plot_lagtime(data_index, values, max_time = 999, step = 10, figax = False): #values has to be a list of the different eigenvalues which you want the plot for, if figax = True returns fig,ax
    ...
    for value in values:
        ...
        for i in range(0, max_time, step):  #calculate timescale
            tau = (i+1) * 1e-5
            w,vl,vr = get_eigen(data_index, i+1)  ###TO DO not very efficient to calculate vectors aswell since they are not needed
...
def ck_test(data_index, lag, set_A, k_range, plot = False):
    ...
    def err_MD(k):
        C = get_count(traj,k)
        return np.sqrt(k*(p_MD(k)-p_MD(k)**2)/sum([sum([C[i][j] for j in range(shape[0]*shape[1])]) for i in set_A]))
...
def entroprod_matrix(data_index):
    ...
    states_h = [np.arange(x_min+1/(2*num_states[0])*(x_max-x_min),x_max,(x_max-x_min)/num_states[0]),
    ...
    for start in range(N):
        for end in range(N):
            ...
            if val > x_max/2:
                ...
                dist = val
            S[start][end] = (-f  * dist * 0.97 + potentialeval(states[start]) - potentialeval(states[end]))/T #0.945 is a manual factor
...
def state_assignement(potential, data_index, threshhold = 0.1, disp = True):
    ...
    guess = [np.linspace(x_min,x_max,int(num_states[0]/2+1)),np.linspace(y_min,y_max,int(num_states[1]/2+1))]
...
def first_passage(start, end, T, length):
    ...
    for i in range(N_end):
        T_a[end[i],:] = 0 
        T_a[end[i],end[i]] = 1; # final points are now absorbing
...
def plot_stat_distr(ref_data, target_data, lagtime, states): # states = [from,to] describes the range in terms of the microstates
...
def plot_local_entprod(ref_data, target_data, lagtime, start):
    ...
    #plotting
    fig,ax = plt.subplots(dpi = 200)
    p = ax.imshow(diff_rew, interpolation="nearest", origin = "lower", cmap = 'coolwarm',vmin=-.5, vmax=.5)

    fig.colorbar(p ,shrink = 0.3, extend ='both');
    ax.plot(np.mod(start,shape[0]),int(start/shape[0]), marker = 'x', markersize = 5, linestyle = '', label = 'starting point')
    ax.legend(loc = 'best', fontsize = 8)
    ax.set_xlabel('No. of micro-state on x-axis', fontsize = 8) 
    ax.set_ylabel('No. of micro-state on y-axis', fontsize = 8) 
    ax.tick_params(axis='both', labelsize = 8)
    ...
    fig,ax = plt.subplots(dpi = 200)
    p = ax.imshow(diff_sim, interpolation="nearest", origin = "lower", cmap = 'coolwarm',vmin=-.5, vmax=.5)
...
def plot_MFPTs(ref_data, target_lst, lagtime, length, start, end): #target_lst describes a list of all targets, start and end are the indices of the different wells
...
def plot_MFPTs_error(ref_data, target_lst, lagtime, length, start, end, N, figax = False): #target_lst describes a list of all targets, start and end are the indices of the different wells, N is the amount of data used for the calculation
    ...
    #plotting
    ax.errorbar(np.arange(len(targetlst)), mu_sim, yerr = err_sim, linestyle = '', marker = '.',
                label = 'simulation', capsize = 2, markersize = 1)
    ax.errorbar(np.arange(len(target_lst)), mu_rew, yerr = err_rew, linestyle = '--', marker = 'x',
                label = 'reweighting', capsize = 0, markersize = 1)
...
def plot_globalent_error(ref_data, target_data_lst, lagtime, N, figax = False): #N is the amount of data used for the calculation
    ...
    #plotting
    fig, ax = plt.subplots(dpi = 150)
    ax.plot(np.arange(len(target_data_lst)), mu_sim, linestyle = '', marker = '.',
                label = 'simulation')
    ax.plot(np.arange(len(target_data_lst)), mu_rew, linestyle = '--',
                label = 'reweighting')
...
def plot_FPTD(ref_data, target_data, lagtime, length, start, end, figax = False): #start and end are the indices of the different wells
            ...
            for start_idx in range(len(min_states)):
                for end_idx in range(len(min_states)):
                    if start_idx != end_idx:
                        ...
                        with open('../data/FPT/FPT_{}_{}{}.txt'.format(ref_data,start_idx,end_idx), mode = 'w+') as file:
                            ...
                            for target_data in data_lst:
                                ...
                                skew_fpt_sim = (np.dot(x**3,fpt_sim) - 3 * sig_fpt_sim**2 * mu_fpt_sim - mu_fpt_sim**3)/sig_fpt_sim**3
                                skew_fpt_rew = (np.dot(x**3,fpt_rew) - 3 * sig_fpt_rew**2 * mu_fpt_rew - mu_fpt_rew**3)/sig_fpt_rew**3

                                #save in file
                                file.write('{}, {}, {}, {}, {}, {} \n'.format(mu_fpt_sim, mu_fpt_rew, sig_fpt_sim, sig_fpt_rew, 
                                                                             skew_fpt_sim, skew_fpt_rew))



##### plot the FPT moments if calculated beforehand #####    
def plot_FPT_moments(data_index, start, end, MFPT = True, sig = False, skew = False):
    file = '/home/theorie/wittenstein/data/FPT/FPT_{}_{}{}.txt'.format(data_index,start,end) #data path needs to be adjusted
