...
//	for (int i =0 ; i < number_counts ; i++)
//	{
//		countfac_v[i] = i * number_steps; // adjust...maybe.
//	}	
//	countfac_v[0] = 1;
...
//	}
//	init_single_particle_acc( x, p, acc, box, dim);
	...
	//	else
	//	{	
		//not needed, because T is set equal everywhere, for now.....
...
//	}}
	...
	for (unsigned long long i=0 ; i < int_steps; i++)
	{
		...
		force_nstate_overdamped_2D(f, x, acc, box, U, Uxsq, T, 
			gamma, dt, rng,p,sigma,bc, err, ran, xm, ext_f,potential);
		pos =find_ms(x,MS,box);
		//cout<<x[0] << "\t"<< x[1] << "\t"<< voronoi[pos][0] << "\t"<< voronoi[pos][1] << "\t"<<  pos << endl;
...
//	vector < vector < vector <int> > > Count_sum;
	int* Count_sum;

//	vector < vector <int> > histo_sum;
	int* histo_sum;
	if (rank ==0)
	{
		Count_sum = new int[number_counts*ms*ms];
		histo_sum = new int[number_counts*ms];
//		Count_sum.resize(number_counts);
//		histo_sum.resize(number_counts);
//		for (int i=0 ; i < number_counts ; i++)
//		{
//			Count_sum[i].resize(ms);
//			histo_sum[i].resize(ms,0);
//			for (int j=0 ; j< ms; j++)
//			{
//				Count_sum[i][j].resize(ms,0);
//			}
//		}
	}
...
/*	
 Shit does not work here, dont know why	
		ofstream res_file;
