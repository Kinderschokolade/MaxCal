...
for i in range(cols):
    ...
    if lagtime[i] == tau:
        ...
        for k in range(lvl):
            for t in range(lvl):
                if k != t:
                    ...
                    mom_con[k * lvl + t, 1] = np.sqrt(
                        mom_con[k * lvl + t, 1] - mom_con[k * lvl + t, 0] * mom_con[k * lvl + t, 0]
                    )  # standard deviation
                    mom_con[k * lvl + t, 2] = (
                        mom_con[k * lvl + t, 2]
                        - 3.0 * mom_con[k * lvl + t, 0] * mom_con[k * lvl + t, 1] * mom_con[k * lvl + t, 1]
                        - mom_con[k * lvl + t, 0] * mom_con[k * lvl + t, 0] * mom_con[k * lvl + t, 0]
                    ) / (
                        mom_con[k * lvl + t, 1] * mom_con[k * lvl + t, 1] * mom_con[k * lvl + t, 1]
                    )  # skewness
                    mom_symm[k * lvl + t, 1] = np.sqrt(
                        mom_symm[k * lvl + t, 1] - mom_symm[k * lvl + t, 0] * mom_symm[k * lvl + t, 0]
                    )  # standard deviation
                    mom_symm[k * lvl + t, 2] = (
                        mom_symm[k * lvl + t, 2]
                        - 3.0 * mom_symm[k * lvl + t, 0] * mom_symm[k * lvl + t, 1] * mom_symm[k * lvl + t, 1]
                        - mom_symm[k * lvl + t, 0] * mom_symm[k * lvl + t, 0] * mom_symm[k * lvl + t, 0]
                    ) / (
                        mom_symm[k * lvl + t, 1] * mom_symm[k * lvl + t, 1] * mom_symm[k * lvl + t, 1]
                    )  # skewness
        ...
        np.savetxt(
            path0 + "single_particle/" + str(int(tau)) + "/MFPT/markovana_con_" + str(ident) + ".dat",
            np.transpose(fpt_ana_con),
        )
        np.savetxt(
            path0 + "single_particle/" + str(int(tau)) + "/MFPT/markovana_symm_" + str(ident) + ".dat",
            np.transpose(fpt_ana_symm),
        )
        np.savetxt(
