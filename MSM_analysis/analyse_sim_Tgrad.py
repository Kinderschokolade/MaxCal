...
import numpy as np
from define_flow import def_flow_direction, read_cut, read_minpos
...
for i in range(c.shape[1]):
    ...
    if i == tau:
        ...
        for k in range(Enum):
            ...
            # 					Evecr_comp[l,1] = 0.
            np.savetxt(
                path0 + "Tgrad/" + str(int(tau)) + "/EV/Evec" + str(k + 1) + "_com_" + str(ident) + ".dat", Evec_comp
            )
            np.savetxt(
                path0 + "Tgrad/" + str(int(tau)) + "/EV/Evecr" + str(k + 1) + "_com_" + str(ident) + ".dat", Evecr_comp
            )
