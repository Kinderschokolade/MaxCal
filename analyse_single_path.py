	...
	if stuff.strip():
		...
		if( pos_old != ms and pos!= ms):
			...
			for p in range(1,lag): # length-1 steps
				...
				SprodC += (traj[p] - traj[p-1])*(traj[p]-traj[p-1])/(dT*2.) + V_l[p]*dT
				SprodA += (traj[p] - traj[p-1])*(F_l[p-1]+ fac1*ran_l[p]  )/(Temperature)
				SprodB += (traj[p] - traj[p-1])*(F_l[p-1] )/(Temperature)
	
			Sprodg = Sprod1 + Sprod2 + Sprod3
			Sprod_data[pos_old][pos].append(Sprodg)
			Sprod1_data[pos_old][pos].append(Sprod1)
			Sprod2_data[pos_old][pos].append(Sprod2)
			Sprod3_data[pos_old][pos].append(Sprod3)
			SprodA_data[pos_old][pos].append(SprodA)   
			SprodB_data[pos_old][pos].append(SprodB)   
			SprodC_data[pos_old][pos].append(SprodC)   
			if (pos == 12 and pos_old == 7 and SprodC < Amin):
				Amin = SprodC
				print(count,SprodA/lag, Sprodg/lag, SprodC,Sprod_th[12][7])
				np.savetxt("/data/pckr194/bause/traj/tr_"+str(count)+".dat",traj)
				count = count +1			
