...
int main(int argc, char *argv[])
{
	...
	double seed;
	read_flags(argc, argv, U ,T , lvl, lag, gamma, ident,fullT, dt,potential,ext_f, ierr,it,ms, write_freq,seed);
...
////////////////////
	...
	for (int i=1 ; i< lvl ; i++)
	{
		...
		{
			xc[i] = double(i)/lvl+findroot(beta,beta,U[i-1],U[i],k,k,int_cut);
		}
	}
	if (potential == 2)
	{
		xc[0] = 0.+findroot(beta,beta,U[lvl-1],U[0],k,k,int_cut);
		xc[lvl] = 1.+xc[0];
	}
	if (potential == 1)
	{
...
//	vector <double> Fvec = get_Fgauss(200,  k, U, xc,boxsize);
//	vector <double> Uvec = get_Ugauss(200,  k, U, xc,boxsize);
//	vector < double> Uvec_ms = get_Ugauss(ms,k,U,xc,boxsize);
	for (int i=0 ; i < 200 ; i++)
	{
		double temp = i/200.;
...
//	exit(EXIT_FAILURE);
	...
	{
		//force_nstate_overdamped_floc(f, x, xo, box, U, T, gamma, dt, rng,p,k,bc, err, ran, xc, ext_f,potential);
		force_nstate_overdamped(f, x, xo, box, U, T, gamma, dt, rng,p,k,bc, err, ran, xc, ext_f,potential);
		//force_nstate(f, x, xo, box, U, T, gamma, dt, rng,p,k,bc, err, ran, xc, ext_f);

		//file<< "pos" <<endl;
		pos =find_ms(x[0],ms);
	
		//pos =find_ms(x[0],ms,shift);  NOTE: Something is going wrong at position pos = 0 here. Check stationary dist
...
//TEST: error in Sprod at boundary comes from approximation at this very point due to potential ? 
	//	else{Sprod_l.push_back(0.);}  //source of error, only for now
		Sprod_l.erase(Sprod_l.begin());
		Sprod = 0;
		bco = bc;
			...
			if (pos_l[1] != ms) // only core states matter
			{
				...
				if (pos != ms) // only core sates matter
				{
					...
					if (i % lag[c] == 0)
					{
						...
						//double expS = exp(-Sprod); //take \pi into account? 
						//Count[c][pos][prev_pos[c]] += exp(-Sprod);
